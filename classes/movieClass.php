...
class movie{
	...
	public function newMovie($title)
	{	


		$movieId = str_replace(" ", "+", $title);
		$movieId = mysqli_real_escape_string($this->conn, $movieId);//in case of apostrophes etc.
		...
		$director = "";
		  ...
		  // throw new Exception("Cannot load xml source.\n");
			...
			//$genre = $omdbXml->movie['genre'];
	      	$director = mysqli_real_escape_string($this->conn, htmlentities(@$omdbXml->movie['director']));
	      	//$releaseDate = $omdbXml->movie['released'];
	      	$writer = mysqli_real_escape_string($this->conn, htmlentities(@$omdbXml->movie['writer']));
	      	$language = @$omdbXml->movie['language'];
	      	$country = @$omdbXml->movie['country'];
	      	$actors = mysqli_real_escape_string($this->conn, htmlentities(@$omdbXml->movie['actors']));
		  	...
		  	$plot= mysqli_real_escape_string($this->conn, @$omdbXml->movie['plot']);
		...
		if(!IsEmpty($imdbID)){
			...
			if(IsEmpty($plot)){
				$plot = mysqli_real_escape_string($this->conn, $data['data']['movies'][0]['plot']);
			} 
		...
		$tomatoInsertQuery = "INSERT INTO `tomato` (`movieId`,`tomatoMeter`, `tomatoImage`, `tomatoRating`, `tomatoReviews`, `tomatoFresh`, `tomatoRotten`, `tomatoUserMeter`,`tomatoUserRating`, `tomatoUserReviews`) VALUES ('$movieId', '$tomatoMeter', '$tomatoImage', '$tomatoRating', '$tomatoReviews', '$tomatoFresh', '$tomatoRotten','$tomatoUserMeter', '$tomatoUserRating', '$tomatoUserReviews')";
		 ...
		 mysqli_query($this->conn, $movieInfoInsertQuery) or die(mysqli_error($this->conn));
		 echo "info d  ";
          mysqli_query($this->conn, $movieNameInsertQuery)or die(mysqli_error($this->conn)); echo "name d  ";
          mysqli_query($this->conn, $imdbInsertQuery)or die(mysqli_error($this->conn)); echo "imdb d  ";
          mysqli_query($this->conn, $linksInsertQuery)or die(mysqli_error($this->conn)); echo "links d  ";
          mysqli_query($this->conn, $tomatoInsertQuery)or die(mysqli_error($this->conn)); echo "tomato d <br>";
} 
}
